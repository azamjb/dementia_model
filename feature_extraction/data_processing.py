...
#                     # Extract baseline and syntactic features
#                     baseline = extract_baseline_features(text)
#                     syntactic = extract_pragmatic_features(text)

#                     # Combine both feature sets
#                     features = {**baseline, **syntactic}
...
def process_transcripts(folder_path):
    ...
    # Loop directly through all .txt files in the single folder
    for filename in os.listdir(folder_path):
        if filename.endswith(".txt"):
            ...
            # Extract baseline, syntactic, and pragmatic features
            baseline = extract_baseline_features(text)
            pragmatic = extract_pragmatic_features(text)

            # Combine all features
            features = {**baseline, **pragmatic}

            # Add metadata
            features["filename"] = filename
            features["label"] = "control"
...
if __name__ == "__main__":
    ...
    folder_path = "../transcripts/no_dementia"
    df = process_transcripts(folder_path)
    print(df.head)

    df.to_csv("control_baseline+pragmatic_features.csv", index=False)
